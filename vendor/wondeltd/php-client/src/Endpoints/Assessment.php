...
class Assessment extends BootstrapEndpoint
{
     ...
     */
    public function __construct($token, $id = false, $logPath = '/logs')
    {
        $this->token = $token;
        ...
        $this->templates = new Templates($token, $this->uri, $logPath);
        $this->aspects = new Aspects($token, $this->uri, $logPath);
        $this->marksheets = new MarkSheets($token, $this->uri, $logPath);
        $this->results = new Results($token, $this->uri, $logPath);
        $this->resultsets = new ResultSets($token, $this->uri, $logPath);
    }
}